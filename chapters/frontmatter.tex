...
\item[ATM] Automated Teller Machine (Guichet Automatique Bancaire)
\item[BRD] Business Requirement Description (Description des Besoins Métier)
\item[BSD] Business Solution Description (Description de la Solution Métier)
\item[DB] Data Base (Base de Données)
\item[E2E] End-To-End (Test de bout en bout)
\item[EMSI] Ecole Marocaine des Sciences de l'Ingénieur
\item[FTP] File Transfer Protocol (Protocole de Transfert de Fichiers)
\item[FTPS] File Transfer Protocol Secure (Protocole de Transfert de Fichiers Sécurisé)
\item[HPS] Hightech Payment Systems
\item[HSBC] Hong-Kong and Shanghai Banking Corporation
\item[ISTQB] International Software Testing Qualifications Board (Comité International de Qualification du Test Logiciel)
\item[JEE] Java Enterprise Edition (Édition d'Entreprise Java)
\item[JSF] Java Server Faces
\item[JWT] JSON Web Token (Jeton Web JSON)
\item[LLM] Large Language Model (Grand Modèle de Langage)
\item[MCP] Model Context Protocol (Protocole de Contexte de Modèle)
...
\item[SFTP] SSH File Transfer Protocol (Protocole de Transfert de Fichiers SSH)
\item[SPA] Single Page Application (Application Web Monopage)
\item[SQL] Structured Query Language (Langage de Requêtes Structuré)
\item[SSE] Server-Sent Events (Événements Envoyés par le Serveur)
\item[UI] User Interface (Interface Utilisateur)
\item[UNIX] Unix
\item[WS] Web Service (Service Web)
