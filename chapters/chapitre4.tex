...
% Nodes
\node[user] (user) {User / QA Engineer};

\node[spa, below left=1.5cm and 0.5cm of user] (react) {React Dashboard SPA\\ \scriptsize Vite :5175};
\node[spa, fill=n8nBlue!20, draw=n8nBlue!80!black, below right=1.5cm and 0.5cm of user] (n8n) {n8n Automation Engine\\ \scriptsize Port :5678};

\node[ai, below=1.5cm of react] (mistral) {Mistral AI Service\\ \scriptsize LLM API};
\node[gw, below right=1.5cm and 1cm of react] (gw) {API Gateway\\ \scriptsize Port :8000};

\node[svc, below left=1.5cm and 1cm of gw] (ref) {Referential Service\\ \scriptsize :8001};
\node[svc, right=0.5cm of ref] (gen) {Generator Service\\ \scriptsize :8002};
\node[svc, right=0.5cm of gen] (runner) {Runner Service\\ \scriptsize :8003};
\node[svc, right=0.5cm of runner] (connect) {Connect Service\\ \scriptsize :8004};

\begin{scope}[on background layer]
    \node[rectangle, draw=yellow!60!black, fill=bgYellow, inner sep=15pt, fit=(ref)(gen)(runner)(connect), label={[font=\sffamily\bfseries\small, text=black!70]above:Microservices Grid (Backend)}] (grid) {};
\end{scope}

\node[db, below=1.5cm of ref] (db) {SQLite Database\\ \scriptsize qa-automation.sqlite};
\node[cy, below right=1.5cm and -0.5cm of gen] (cypress) {Cypress Engine};

% Edges
\draw[arw] (user) -- node[lbl]{Uses UI} (user |- react.north);
\draw[arw] (react.south) ++(-1,0) coordinate(r_ai) -- node[lbl]{AI Discovery \& Generation} (r_ai |- mistral.north);
\draw[arw] (react.south) ++(1,0) coordinate(r_gw) -- node[lbl]{API Requests} (gw.north west);

\draw[arw] (n8n.south) -- node[lbl]{Data Prep / Archiving} (gw.north east);
...
\draw[arw, <->] (ref.south) -- node[lbl]{Reads/Writes SQL} (db.north);
\draw[arw] (gen.south) -- node[lbl, left]{\scriptsize Generates .cy.js files} (cypress.north west);
\draw[arw] (runner.south) -- node[lbl, right]{\scriptsize Spawns Test Process} (cypress.north east);
...
\begin{tikzpicture}[node distance=1cm and 2cm]
  ...
  \node[user, fill=orangeC!20, draw=orangeC!80!black, below=10cm of tester] (admin) {Admin};

  \node[spa, right=3cm of tester, yshift=2cm] (uc1) {UC1: Sélectionner un écran};
  ...
  \node[svc, fill=orangeC!10, draw=orangeC!80, right=3cm of admin, yshift=1cm] (uc12) {UC12: Scanner specs};
  ...
  \begin{scope}[on background layer]
    \node[bxBlue, fit=(uc1)(uc11)(uc12)(uc14), label={[lB]above:QA Cypress Dashboard}] {};
  \end{scope}
  ...
  \draw[darw, color=purpleC] (uc5.east) -- ++(1,0) |- node[lbl, near start]{\scriptsize\guillemotleft include\guillemotright} (uc2.east);
  \draw[darw, color=purpleC] (uc8.east) -- ++(1.5,0) |- node[lbl, near start]{\scriptsize\guillemotleft include\guillemotright} (uc5.east);
  \draw[darw, color=purpleC] (uc9.east) -- ++(2,0) |- node[lbl, near start]{\scriptsize\guillemotleft include\guillemotright} (uc8.east);
\end{tikzpicture}
}%
...
\begin{tikzpicture}[node distance=1.5cm and 2cm]
  ...
  \node[svc, below left=2.5cm and 3cm of gw] (ref) {\textbf{Referential Service}\\ \scriptsize :8001};
