...
\begin{tikzpicture}
  ...
  \node[svc, minimum width=2.5cm, right=2.5cm of dash] (api) {\textbf{API Server}};
  \node[svc, fill=orangeC!10, draw=orangeC!80!black, minimum width=2.5cm, right=2.5cm of api] (connect) {\textbf{Connect API}};
  \node[db, minimum width=2.5cm, right=2.5cm of connect] (fs) {\textbf{File System}};
  ...
  \draw[arw, accent, dashed] ([yshift=-14cm]api.south) -- node[lbl, above]{14. success} ([yshift=-14cm]dash.south);
\end{tikzpicture}
}%
...
\begin{tikzpicture}
  ...
  \node[svc, minimum width=2.5cm, right=3cm of dash2] (runner2) {\textbf{Runner Service}};
  \node[cy, draw=orangeC!80!black, fill=orangeC!10, minimum width=2.5cm, right=3cm of runner2] (cypress2) {\textbf{Cypress Process}};
  ...
  \draw[arw, blueC] ([yshift=-6cm]dash2.south) -- ++(0.5,0) |- ([yshift=-6.6cm]dash2.south) node[lbl, right, yshift=0.3cm]{\footnotesize parseCypressLogs() $\rightarrow$ rapport visuel};

  \draw[arw, orangeC, dashed] ([yshift=-8cm]cypress2.south) -- node[lbl, above]{6. exit code} ([yshift=-8cm]runner2.south);
  \draw[arw, accent] ([yshift=-8.6cm]runner2.south) -- node[lbl, above]{7. event: end} ([yshift=-8.6cm]dash2.south);
  \draw[arw, accent, dashed] ([yshift=-9.2cm]dash2.south) -- node[lbl, above]{8. Résumé final} ([yshift=-9.2cm]user2.south);
\end{tikzpicture}
}%
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}
  \node[svc, fill=greenC!10, draw=greenC!80!black, minimum width=2.5cm] (vite) {\textbf{Vite Dev Server}};
  \node[svc, minimum width=2.5cm, right=3cm of vite] (catalog) {\textbf{catalog.mjs}};
  \node[db, minimum width=2.5cm, right=3cm of catalog] (fsys) {\textbf{File System}};
  \node[spa, minimum width=2.5cm, right=3cm of fsys] (output) {\textbf{JSON Output}};
...
\begin{tikzpicture}[node distance=1.5cm and 2cm]
  ...
  \node[gw, below=2cm of client] (gw) {\textbf{API Gateway}\\ \scriptsize localhost:8000};

  \node[svc, below left=2.5cm and 3cm of gw] (ref) {\textbf{Service Référentiel}\\ \scriptsize :8001};
  \node[svc, right=1cm of ref] (gen) {\textbf{Generator Service}\\ \scriptsize :8002};
  \node[svc, right=1cm of gen] (run) {\textbf{Runner Service}\\ \scriptsize :8003};
  \node[svc, right=1cm of run] (conn) {\textbf{Connect Service}\\ \scriptsize :8004};
